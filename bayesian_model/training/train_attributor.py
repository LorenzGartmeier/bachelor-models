...
for folder in folder_names:
    ...
    dataset = get_dataset(os.path.join('bayesian_model/self_descriptions', folder, 'train/*.npy'))
    ...
    history_df.to_csv(f"bayesian_model/training/results/gmm_loss_{folder}.csv", index=False)
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"bayesian_model/training/results/gmm_loss_{folder}.png", dpi=300)
    plt.close()

    i += 1

attributor.save('bayesian_model/saved_weights/attributor')
